...
export default function DroppableColumn({id, data, tittle}){
    return (
        ...
        {
            (provided, snapshot) => (
            <div ref={ provided.innerRef } { ...provided.droppableProps } className={ `container max-w-xs mx-2 rounded-lg flex-1 mb-2 min-h-[10rem] ${snapshot.isDraggingOver ? 'border-2 border-dotted border-bright-turquoise-600' : 'border-2'}` }
            >
